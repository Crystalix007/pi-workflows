  ...
  const rpc = (method: string, params: unknown) =>
    new Promise((resolve, reject) => {
      ...
      const offRet = pi.events.on(REPLY_PREFIX + requestId, (reply: any) => {
        ...
        else reject(new Error(`${method} failed: ${JSON.stringify(reply?.error)}`));
      });
  ...
  const pollUntilDone = async (data: any, label: string) => {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (state === "complete" || state === "failed") {
        log(`${label}: terminal (${state}); details.results = ${JSON.stringify(s?.details?.results).slice(0, 1200)}`);
        log(`${label}: text tail = ...${text.slice(-300)}`);
        return s;
      }
    }
    log(`${label}: no terminal state seen; LAST raw ===\n${JSON.stringify(last).slice(0, 2200)}`);
  };
