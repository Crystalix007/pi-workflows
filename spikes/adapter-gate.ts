...
const [r1] = await runtime.run(
	P +
		`return prompt("decide", schema{ correct = bool, reasoning = str }):await()`,
);
check(
	"prompt+structured-output returns parsed object",
	!!r1 &&
		(r1 as any).correct === true &&
		(r1 as any).reasoning === "mock-reasoning",
	JSON.stringify(r1),
);
...
const [r3] = await runtime.run(
	P + `return subagent{ agent="worker", task="fix bug #42" }:await()`,
);
check(
	"subagent text",
	(r3 as any)?.text === "mock-subagent[worker]: fix bug #42",
	JSON.stringify(r3),
);

// ---- Test 4: subagent with explicit context ----
const [r4] = await runtime.run(
	P +
		`return subagent{ agent="reviewer", task="review diff", context="fresh" }:await()`,
);
check(
	"subagent explicit context",
	(r4 as any)?.text?.startsWith("mock-subagent[reviewer]"),
	JSON.stringify(r4),
);
