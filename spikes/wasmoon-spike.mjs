...
try {
  ...
  const tbl = (await run(`return { reasoning = "ok", complete = true, items = { 10, 20, 30 } }`))[0];
  console.log("  ->", JSON.stringify(tbl), "| complete:", tbl.complete, `(${typeof tbl.complete})`);

  log(2, "JS function called from Lua");
  lua.global.set("add", (x, y) => x + y);
  console.log("  -> add(3,4) =", (await run(`return add(3, 4)`))[0]);

  log(3, "await JS Promise from Lua via :await()  (exercises run() yield/resume)");
  lua.global.set("asyncStep", (label, ms) =>
